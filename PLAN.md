...
| --- | --- | --- | --- |
| REL-01 | `v0.1.0` release-candidate audit | Independent validation pending | The npm tarball includes the MIT license and intended source/runtime files without build output, tests, secrets, keys, certificates, source maps, or environment files. Windows validation passes locally. Independent macOS and Linux reports remain required. |
...
| L-06 | macOS support | Independent validation pending | Volunteers compile the WKWebView host and Mach-O launcher, package `.app`/DMG output, and run unit, live bridge, and multi-window tests. Optional Developer ID signing/notarization remains configuration-dependent. |
| L-07 | Linux support | Independent validation pending | Volunteers compile the GTK 3/WebKitGTK 4.1 host and POSIX launcher, package the portable app folder, and run unit, live bridge, and multi-window tests, using Xvfb when needed. |
| L-08 | Independent macOS/Linux validation | Ready for contributors | `PLATFORM-TESTING.md` defines the priority OS/architecture matrix, reproducible build and live-test commands, package smoke checks, evidence requirements, and completion criteria. Public reports use ordinary repository issues. |
...
| SEC-07 | Security release gate | Windows complete; parity deferred | The repeatable Windows `security:gate` runs repository and package-surface secret and hidden-character scanning, production dependency audit, warning-as-error MSVC analysis, PE hardening verification, malformed IPC/integrity corpora, updater/package-input/package-tamper tests, and installer smoke. `SECURITY-CHECKLIST.md` inventories privileged APIs and blocks releases with high/critical findings or undocumented capabilities. macOS/Linux parity is deferred. |
...
- The macOS implementation includes a WKWebView host, native launcher, `.app` and DMG packaging, WebKit bridge transport, and multi-window routing; independent validation is pending.
- Linux includes a GTK 3/WebKitGTK 4.1 native host, POSIX launcher, and portable app-folder packager; independent validation is pending. Runtime paths use `XDG_DATA_HOME` (or `~/.local/share`) and isolate WebKitGTK profiles by stable app id.
